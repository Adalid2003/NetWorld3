        ...
        if (request.ok) {
            request.json().then(function (response) {
                ...
                if (response.status) {
                    ...
                    document.getElementById('nombres_perfil').value = response.dataset.nombre_usuario;
                    ...
                    document.getElementById('alias_perfil').value = response.dataset.apodo_usuario;
...
function inactividad() {
    swal({
        title: 'Advertencia',
        text: 'Su sesión será cerrada por inactividad',
        icon: 'warning',
        buttons: ['Ok', 'Seguir en la sesión'],
        closeOnClickOutside: false,
        closeOnEsc: false
    }).then(function (value) {
        // Se verifica si fue cliqueado el botón Sí para hacer la petición de cerrar sesión, de lo contrario se muestra un mensaje.
        if (value) {
            fetch(API + 'inactividad', {
                method: 'get'
            }).then(function (request) {
                // Se verifica si la petición es correcta, de lo contrario se muestra un mensaje indicando el problema.
                if (request.ok) {
